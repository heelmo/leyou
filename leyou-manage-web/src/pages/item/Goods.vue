    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="saleableGoods(props.item.id,false)">下架</v-btn>
          <v-btn icon v-else @click="saleableGoods(props.item.id,true)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      saleableGoods(spuId,saleable) {
            this.$http.put(`/item/spu/${spuId}/saleable/${saleable}`)
            .then(() => {
                this.getDataFromServer();
            })
      },
      deleteGoods(spuId) {
        ...
        .then(() => {
            this.$http.delete("/item/spu/" + spuId)
            .then(() => {
